...
/* ================================================================
 *  内部：解析一条完整的帧字符串
 *  输入示例: "SA,1,90,120,120,60,120,180,120,100,80,140,100,ED"
 * ================================================================ */
static bool parse_frame(const char *raw, K230_Frame_t *frame)
{
    ...
    frame->node1_x    = (int16_t)fields[8];
    frame->node1_y    = (int16_t)fields[9];
    frame->node2_x    = (int16_t)fields[10];
    frame->node2_y    = (int16_t)fields[11];
    /* fields[12] 是 ED 前最后一个数，实际协议里无第13字段，留空 */
...
void K230_FrameToStr(const K230_Frame_t *frame, char *out, uint16_t out_size)
{
    ...
    snprintf(out, out_size,
             ...
             frame->node1_x, frame->node1_y,
             frame->node2_x, frame->node2_y);
}
